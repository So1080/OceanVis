...
HABITAT_SOUND_PATHS = {
    ...
    "zone_complete":      "sounds/zone_complete.mp3",
}
        ...
        if app_state == "ZONE_COMPLETE":
            ...
            if pt1 and pt2:
                ...
                if all(all(row) for row in revealed_grid) and not active_zone["completed"]:
                    ...
                    if "zone_complete" in LOADED_SOUNDS:
                        EVAL_CHANNEL.play(LOADED_SOUNDS["zone_complete"])
                    elif "correct" in LOADED_SOUNDS:
                        EVAL_CHANNEL.play(LOADED_SOUNDS["correct"])
