...
# NEW: Manual Offset for Homography (Top-Left Corner)
TL_OFFSET_X = 0
TL_OFFSET_Y = 0

# NEW: Manual Offset for Homography (Bottom-Right Corner)
BR_OFFSET_X = 0
BR_OFFSET_Y = 0
    ...
    if should_update_homography:
        for i, marker_id in enumerate(ids.flatten()):
            if marker_id in HOMOGRAPHY_MARKER_IDS:
                ...
                # Define map_marker_pts with the manual offsets applied to TL, BR, and BL points
                ...
                if detected_color != state['last_color']:
                    ...
                    if detected_color:
                        ...
                        if sound_key and sound_key in SOUNDS:
                            # MODIFIED: Added loops=-1 for continuous sound
